...
import type { AgentOutput, AnalysisResponse, Classification, Holding, MarketSnapshot, ProfileInput, RiskProfile, Source } from "@/types/analysis";
...
export function Dashboard() {
  ...
  const [holdings, setHoldings] = useState<Holding[]>([{ symbol: "TCS", weight: 70 }, { symbol: "RELIANCE", weight: 30 }]);
  ...
  const allocation = holdings.reduce((sum, holding) => sum + Number(holding.weight || 0), 0);
  const profilePayload = useMemo<ProfileInput>(() => ({ user_id: USER_ID, risk_profile: risk, investment_horizon_years: horizon, maximum_volatility: maxVolatility, portfolio: holdings, watchlist, interaction_history: [{ action: "viewed", symbol: selectedSymbol }] }), [risk, horizon, maxVolatility, holdings, watchlist, selectedSymbol]);
  const validation = allocation !== 100 ? `Portfolio allocation must total 100%. Current total: ${allocation}%.` : horizon < 1 || horizon > 50 ? "Investment horizon must be between 1 and 50 years." : maxVolatility < 0 || maxVolatility > 100 ? "Maximum volatility must be between 0% and 100%." : "";
  ...
  async function handleSave(showSuccess = true) {
    if (validation) { setMessage({ kind: "error", text: validation }); return false; }
    ...
    catch (error) { setConnected(false); setMessage({ kind: "error", text: error instanceof Error ? error.message : "Unable to save the profile." }); return false; }
  ...
  async function handleAnalyze() {
    ...
    catch (error) { setConnected(false); setMessage({ kind: "error", text: error instanceof Error ? error.message : "Analysis failed. Please retry." }); }
    finally { setLoading(false); }
  }

  function updateHolding(index: number, field: "symbol" | "weight", value: string | number) { setHoldings((current) => current.map((holding, itemIndex) => itemIndex === index ? { ...holding, [field]: field === "weight" ? Number(value) : value } : holding)); }
    ...
    <section className="command-grid">
      ...
      <section className="research-column">
        ...
        <div className="analysis-control"><div className="control-title"><div><span className="pulse-mark">✦</span><div><h2>Run a new analysis</h2><p>Uses the profile and portfolio shown at left.</p></div></div><span>4 agents</span></div><label htmlFor="company">Company</label><select id="company" value={selectedSymbol} onChange={(event) => setSelectedSymbol(event.target.value)} disabled={loading}>{stocks.length ? stocks.map((stock) => <option key={stock.symbol} value={stock.symbol}>{stock.symbol} · {stock.company_name.replace(" (Simulated)", "")}</option>) : SYMBOLS.map((symbol) => <option key={symbol}>{symbol}</option>)}</select><button className="primary-button" onClick={() => void handleAnalyze()} disabled={loading || Boolean(validation)}>{loading ? "Agents are researching…" : "Run Multi-Agent Analysis"}<span>{loading ? "◌" : "→"}</span></button>
          ...
          {message && <div className={`feedback ${message.kind}`} role={message.kind === "error" ? "alert" : "status"}><span>{message.kind === "success" ? "✓" : "!"}</span><p>{message.text}</p>{message.kind === "error" && <button onClick={() => connected === false ? void refresh() : void handleAnalyze()}>Retry</button>}</div>}
        </div>
