...
export function Dashboard() {
  const [activeView, setActiveView] = useState<"overview" | "markets" | "agents" | "decision" | "evidence" | "history">("overview");
  const [connected, setConnected] = useState<boolean | null>(null);
  const [version, setVersion] = useState("");
  const [history, setHistory] = useState<AnalysisResponse[]>([]);
  const [decisions, setDecisions] = useState<UserDecision[]>([]);
  const [selectedSymbol, setSelectedSymbol] = useState("");
  const [selectedInstrument, setSelectedInstrument] = useState<Instrument>({ instrument_key: "", exchange: "NSE", symbol: "", name: "", isin: null, instrument_type: "EQ" });
  const [hasChosenInstrument, setHasChosenInstrument] = useState(false);
  ...
  const profilePayload = useMemo<ProfileInput>(() => ({ user_id: USER_ID, risk_profile: risk, investment_horizon_years: horizonNumber, maximum_volatility: volatilityNumber, portfolio: holdings.map((holding) => ({ symbol: holding.symbol, instrument_key: holding.instrumentKey, exchange: holding.exchange, weight: Number(holding.weight) })), watchlist, interaction_history: hasChosenInstrument ? [{ action: "viewed", symbol: selectedSymbol, instrument_key: selectedInstrument.instrument_key }] : [] }), [risk, horizonNumber, volatilityNumber, holdings, watchlist, selectedSymbol, selectedInstrument, hasChosenInstrument]);
  const validation = horizon.trim() === "" ? "Investment horizon is required." : !Number.isFinite(horizonNumber) || !Number.isInteger(horizonNumber) || horizonNumber < 1 || horizonNumber > 50 ? "Investment horizon must be a whole number from 1 to 50 years." : maxVolatility.trim() === "" ? "Maximum volatility is required." : !Number.isFinite(volatilityNumber) || volatilityNumber < 0 || volatilityNumber > 100 ? "Maximum volatility must be a number from 0% to 100%." : holdings.some((holding) => holding.weight.trim() === "" || !Number.isFinite(Number(holding.weight)) || Number(holding.weight) < 0 || Number(holding.weight) > 100) ? "Each portfolio weight must be a number from 0% to 100%." : holdings.length > 0 && Math.abs(allocation - 100) > 0.001 ? `Portfolio allocation must total 100%. Current total: ${allocation.toFixed(2).replace(/\.00$/, "")}%.` : "";
  ...
  const refresh = useCallback(async (signal?: AbortSignal) => {
    ...
    const stored = typeof window === "undefined" ? [] : JSON.parse(window.localStorage.getItem("finsync-analysis-history") ?? "[]") as AnalysisResponse[];
    if (results[1].status === "fulfilled") { const merged = [...results[1].value, ...stored].filter((item, index, all) => all.findIndex((other) => other.analysis_id === item.analysis_id) === index); setHistory(merged); } else setHistory(stored);
  ...
  async function handleAnalyze() {
    if (loading) return;
    if (!hasChosenInstrument || !selectedInstrument) { setMessage({ kind: "error", text: "Search for and select an NSE or BSE instrument first." }); return; }
    const saved = await handleSave(false); if (!saved) return;
    setLoading(true); setStage(0); setMessage(null);
    try { const result = await runAnalysis(USER_ID, selectedSymbol, selectedInstrument.instrument_key); setAnalysis(result); setActiveView("agents"); setConnected(true); setMessage({ kind: "success", text: `${selectedSymbol} analysis completed with ${result.metrics.agents_completed} of ${result.metrics.agents_expected} agents.` }); const stored = JSON.parse(window.localStorage.getItem("finsync-analysis-history") ?? "[]") as AnalysisResponse[]; const localHistory = [result, ...stored].filter((item, index, all) => all.findIndex((other) => other.analysis_id === item.analysis_id) === index).slice(0, 30); window.localStorage.setItem("finsync-analysis-history", JSON.stringify(localHistory)); const updated = await loadAnalysisHistory(USER_ID); setHistory([...updated, ...localHistory].filter((item, index, all) => all.findIndex((other) => other.analysis_id === item.analysis_id) === index)); window.setTimeout(() => document.getElementById("results")?.scrollIntoView({ behavior: "smooth" }), 50); }
  ...
  function addHolding() { if (!hasChosenInstrument || !selectedInstrument || holdings.some((holding) => holding.instrumentKey === selectedInstrument.instrument_key)) return; setHoldings((current) => [...current, { symbol: selectedInstrument.symbol, instrumentKey: selectedInstrument.instrument_key, exchange: selectedInstrument.exchange, weight: "0" }]); }
  ...
  function chooseInstrument(item: Instrument) { setSelectedInstrument(item); setSelectedSymbol(item.symbol); setHasChosenInstrument(true); setSearchQuery(""); setSearchResults([]); }
  ...
  return <main className="app-shell" id="top">
    ...
    <div className="product-frame">
      <aside className="product-sidebar" aria-label="FinSync features">
        <div className="sidebar-heading"><span>Workspace</span><strong>Research desk</strong></div>
        <nav>{[
          ["overview", "01", "Overview"],
          ["markets", "02", "Market data"],
          ["agents", "03", "Agent research"],
          ["decision", "04", "Decision lab"],
          ["evidence", "05", "Evidence"],
          ["history", "06", "History"],
        ].map(([view, number, label]) => <button key={view} className={activeView === view ? "active" : ""} onClick={() => setActiveView(view as typeof activeView)}><span>{number}</span>{label}</button>)}</nav>
        <section className="allocation-chart" aria-label="Portfolio allocation chart"><span>Portfolio allocation</span><AllocationPie items={holdings.map((holding) => ({ symbol: holding.symbol, weight: Number(holding.weight) || 0 }))}/><div className="pie-legend">{holdings.map((holding) => <small key={holding.instrumentKey}>{holding.symbol} {holding.weight || "0"}%</small>)}</div></section>
        <div className="sidebar-status"><span className={connected ? "status-dot online" : "status-dot"}/><div><strong>{connected ? "Data connected" : "Connecting"}</strong><small>{quote?.provider_name ?? "Market service"}</small></div></div>
      </aside>
      <div className={`product-view view-${activeView}`}>

    <section className="command-grid view-panel overview-panel">
      ...
      <section className="research-column">
        ...
        <section className="market-overview" aria-label="Live market overview">
          <div className="market-overview-copy">
            ...
            <strong>{hasChosenInstrument ? selectedInstrument.symbol : "No selection"}</strong>
            <small>{quote ? `${formatMoney(quote.last_price)} · ${quote.freshness}` : "Loading market quote"}</small>
          </div>
          <div className="price-bars" aria-label="Actual closing-price history graph">
            {candles.length > 1 ? <PriceChart candles={candles}/> : <small>{hasChosenInstrument ? "Loading actual market history" : "Choose any listed NSE or BSE instrument"}</small>}
          </div>
          <div className="market-stat"><span>Research agents</span><strong>04</strong><small>Independent views</small></div>
          <div className="market-stat"><span>Portfolio allocation</span><strong>{allocation.toFixed(0)}%</strong><small>{validation ? "Needs attention" : "Profile ready"}</small></div>
        </section>
        <div className="analysis-control"><div className="control-title"><div><span className="pulse-mark">✦</span><div><h2>Search all Indian equities</h2><p>Type any company name or NSE/BSE symbol, then choose its exact listing.</p></div></div><span>4 agents</span></div><label htmlFor="instrument-search">Company or symbol</label><input id="instrument-search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search HDFC Bank, Zomato, SBIN, Tata Motors…" autoComplete="off"/>{searching && <p className="search-state">Searching catalogue…</p>}{searchQuery.length >= 2 && !searching && !searchResults.length && <p className="search-state">No supported equity instruments found.</p>}{searchResults.length > 0 && <ul className="instrument-results" role="listbox">{searchResults.map((item) => <li key={item.instrument_key}><button role="option" aria-selected={hasChosenInstrument && item.instrument_key === selectedInstrument.instrument_key} onClick={() => chooseInstrument(item)}><strong>{item.symbol}</strong><span>{item.name}</span><b>{item.exchange}</b></button></li>)}</ul>}<div className="selected-instrument"><div><small>Selected instrument</small>{hasChosenInstrument ? <><strong>{selectedInstrument.symbol} · {selectedInstrument.exchange}</strong><span>{selectedInstrument.name}</span></> : <><strong>Search to choose a listing</strong><span>No default stock is preselected.</span></>}</div>{hasChosenInstrument && (quote ? <div><small>{quote.provider_name} · {quote.data_mode}</small><strong>{formatMoney(quote.last_price)}</strong><span>{quote.provider_timestamp ? formatDate(quote.provider_timestamp) : quote.freshness}</span></div> : <div><small>Market rate</small><span>{quoteError || "Loading…"}</span></div>)}</div>{quote?.fallback_reason && <p className="quote-warning">⚠ {quote.fallback_reason}</p>}<button className="primary-button" onClick={() => void handleAnalyze()} disabled={loading || Boolean(validation) || !hasChosenInstrument}>{loading ? "Agents are researching…" : hasChosenInstrument ? "Run Multi-Agent Analysis" : "Choose a stock to continue"}<span>{loading ? "◌" : "→"}</span></button>
    ...
    {!analysis && !loading && <section className="first-use view-panel overview-panel"><span>✦</span><div><p className="eyebrow">Your first report starts here</p><h2>Select a company and run the four-agent pipeline.</h2><p>The complete report will keep agent evidence, conflicts, citations, risk context, and missing inputs visible.</p></div></section>}
    {!analysis && activeView !== "overview" && <section className="feature-empty"><span>{activeView === "markets" ? "Market data" : activeView === "agents" ? "Agent research" : activeView === "decision" ? "Decision lab" : activeView === "evidence" ? "Evidence" : "History"}</span><h2>Run an analysis to populate this workspace.</h2><p>FinSync only charts returned market fields and completed research outputs. It does not invent missing series.</p><button onClick={() => setActiveView("overview")}>Go to overview →</button></section>}
