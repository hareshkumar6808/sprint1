...
import { getHealth, getQuote, getQuotes, loadAnalysisHistory, loadDecisions, recordDecision, runAnalysis, saveProfile as persistProfile, searchInstruments } from "@/lib/api";
import { DECISION_LAB_PREVIEW } from "@/lib/decision-lab-fixture";
import { DecisionLab } from "@/components/DecisionLab";
import type { AgentOutput, AnalysisResponse, Classification, DecisionAction, Instrument, MarketQuote, ProfileInput, RiskProfile, Source, UserDecision } from "@/types/analysis";
...
export function Dashboard() {
  ...
  useEffect(() => { const controller = new AbortController(); setQuote(null); setQuoteError(""); void getQuote(selectedInstrument.instrument_key, controller.signal).then(setQuote).catch((error) => setQuoteError(error instanceof Error ? error.message : "Quote unavailable")); return () => controller.abort(); }, [selectedInstrument]);
